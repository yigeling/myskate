<template>
	<div class="content">
		...
		<van-cell-group title="" style="margin-top: 20px;">
		  <van-cell title="我的主页" is-link to="/uc/mycenter"></van-cell>
	...
	onMounted(()=>{
		...
		.then((res)=>{
			...
			if(res.code == 0){
				...
				userStore.saveName(res.data.username)
				userStore.saveAvator(res.data.avator)
				userStore.saveIntro(res.data.intro)
				userStore.saveAge(res.data.age)
				userStore.saveSex(res.data.sex)
			}else{
