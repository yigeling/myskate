				...
				<div v-for="n in sortedUserPosts" style="margin-top: 5px;">
					...
					<van-row style="margin-top: 2px;">
						...
						<van-col span="3" offset="1">
							<van-icon name="good-job-o" size="20px" @click="Tags(n.id)" />{{n.user_like}}
							<div v-if="show">{{n.id}}</div>
						</van-col>
						<van-col span="1">
	...
	function Tags(userlike_id) {
		...
		.then(res =>{
			if (res.code != 0){
				...
				console.log("点赞了这条动态")
				  const post = userposts.value.find(item => item.id === userlike_id);
				  if (post) { 
				    // 将user_like值加1
				    post.user_like = res.data
				  }
			}
		})
	}
